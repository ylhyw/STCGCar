        ...
        for epoch in tqdm(range(args.epochs)):
            ...
            if epoch % 1 ==0:
                ...
                if ari > best_ari:
                    ...
                    y = pre_labels
                    ture_ari = np.round(metrics.adjusted_rand_score(y, adata.obs['ground_truth']), 5)
                    ture_nmi = np.round(metrics.normalized_mutual_info_score(y, adata.obs['ground_truth']), 5)
                    best_ari = ari
                    best_nmi = nmi
                    best_emb = hidden_emb
            ...
            tqdm.write('best_nmi: {}, best_ari: {} , lr:{} ,loss:{},range:{}'.format(ture_nmi, ture_ari,optimizer.param_groups[0]['lr'],total_loss, i))

        ari_list.append(ture_ari)
        nmi_list.append(ture_nmi)

        if ture_ari > super_best_ari:
            super_best_ari = ture_ari
            best_adata = adata
